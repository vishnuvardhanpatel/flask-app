...
class DataLoader(object):
    ...
    def _get_test_ds(self):
        test_DSs = [P.Path(sub_dir) for sub_dir in P.Path(self.test_ds_dir).glob(f'{self.file_format}/*.{self.file_format}')]

        # test_DSs = [P.Path(sub_dir) for sub_dir in P.Path(self.test_ds_dir).glob(f'{file_format}/*.{file_format}')]
        DS = {}
        for test_ds_dir in test_DSs:
            name = test_ds_dir.name
            test_ds_dir = '\\'.join(str(test_ds_dir).split('\\')[:-1])
            test_ds_dir = P.Path(test_ds_dir)
    ...
    def _get_test_one_ds(self, dir_, img_types, MAX_SAMPLES):
        ...
        ds_test = ds_test.map(
            lambda x: tf.py_function(
                func=_get_image_fn,
                inp=[x, self.WH, self.num_channels, self.conv_func],
...
def _get_image_fn(file_path: tf.Tensor,
                  ...
                  conv_fun):
    #
    if conv_fun == 'png_to_png':
        img = cnv.png_to_png()
    if conv_fun == 'dcm_to_png':
        img = cnv.dcm_to_png()
    if conv_fun == 'dcm_to_jpeg':
        img = cnv.dcm_to_jpeg()
...
def _adding_awgn_gibbs(x: tf.Tensor,
    ...
    if noise_:
        ...
        wgn = np.random.normal(loc=0., scale=stddev, size=_get_length(
            tf.shape(x_distorted)))  # since the other authots used np.normal, we use the same template
